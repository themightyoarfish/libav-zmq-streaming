  ...
  int decode(AVCodecContext *dec_ctx, AVFrame *frame, AVPacket *pkt) {
    ...
    if (ret < 0) {
      std::cerr << "Error sending packet for decoding: "
                << avutils::av_strerror(ret) << std::endl;
      return ret;
    } else {
    ...
    while (ret >= 0) {
      ...
      if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
        /* std::cerr << "Decoder shits bed: " << avutils::av_strerror(ret) <<
         * std::endl;
         */
        return ret;
      } else if (ret < 0) {
        std::cerr << "Error during decoding: " << avutils::av_strerror(ret)
                  << std::endl;
        return ret;
      } else {
